...
public sealed class AfsUdpSessionService(Func<IAfsFrameCodec>? codecFactory = null)
{
    ...
    public async Task<AfsSessionResult> SendAsync(AfsTestSettings test, AfsTransportSettings network, IProgress<AfsSessionProgress>? progress, CancellationToken token)
    {
        ...
        var manifest = new AfsSessionManifest(testId, 1, test.Prn, test.CustomMessageType, sourceLength, sourceHash, records.Count, frames.Count, frames[0].Week, frames[0].Itow, frames[0].Toi, offset);
        ...
        for (var i = 0; i < frames.Count; i++)
        {
            ...
            await SendCopiesAsync(udp, destination, packet, network.RepeatCount, token);
        ...
        while (true)
        {
            ...
            var counters = new AfsNetworkCounters(wire.ExpectedFrames, wire.ReceivedFrames, frames.Count * network.RepeatCount, wire.ReceivedDatagrams, wire.Duplicates, wire.Corrupt, wire.Probes, wire.ProbeResponses, sourceLength, stopwatch.Elapsed, [], wire.AverageLatency, wire.MaximumLatency);
    ...
    public async Task<AfsSessionResult> ReceiveAsync(AfsTestSettings test, AfsTransportSettings network, IProgress<AfsSessionProgress>? progress, CancellationToken token)
    {
        ...
        var frameCount = dedup.Count; var counters = new AfsNetworkCounters(manifest.FrameCount, Math.Min(manifest.FrameCount, latencies.Count), manifest.FrameCount * network.RepeatCount, datagrams, duplicates, corrupt, Math.Max(1, probes), probeResponses, info.Length, started.Elapsed, latencies);
