...
public sealed class AfsProtocolTests
{
    ...
    [Fact]
    public void Sb2ValidationPatternIsDeterministicAndCarriesTimeFields()
    {
        const ushort week = 2300, intervalOfWeek = 321;
        var first = AfsSb2Builder.BuildValidationPattern(week, intervalOfWeek);
        var second = AfsSb2Builder.BuildValidationPattern(week, intervalOfWeek);

        Assert.Equal(1176, first.Length);
        Assert.Equal(first, second);
        Assert.Equal(week, ReadBits(first, 0, 13));
        Assert.Equal(intervalOfWeek, ReadBits(first, 13, 9));
        Assert.Throws<ArgumentOutOfRangeException>(() => AfsSb2Builder.BuildValidationPattern(week, 504));

        // SB2Builder와 동일한 MSB-first 규칙으로 시간 필드를 다시 읽는다.
        static int ReadBits(IReadOnlyList<byte> bits, int offset, int length)
        {
            var value = 0;
            for (var i = 0; i < length; i++) value = (value << 1) | bits[offset + i];
            return value;
        }
    }

    [Fact]
    public async Task NativeCodecRoundTripRequiresDll()
    {
