...
3. Test A~E 중 하나를 선택하면 해당 시험에서 사용하는 라벨과 입력칸만 활성화된다. 활성 입력에 오류 심볼 수, Seed, SP 손상 간격 또는 UDP Drop 조건을 입력하며, 비활성 입력의 기존 값은 유지되지만 현재 시험에는 적용·검증되지 않는다. 비활성 상태에서도 각 항목에 마우스를 올리면 적용 시험·범위·동작 설명이 표시된다.
